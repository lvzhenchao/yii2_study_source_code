...
`
    ...
    php -m
`
# 进程：运行的程序；程序运行的载体；它的上一层是CPU；cpu的1~4倍
...
# 进程和线程的区别
- 线程是程序执行的最小单位，最接近任务执行的；而进程是操作系统分配资源的最小单位，基于cpu分配的
- 一个进程由一个或多个线程组成；线程是一个进程中代码的不同执行路线
- 进程之间是相互独立的；而同一进程下的各个线程之间是共享程序内存空间的【包括代码段，数据集，堆等】
- 调度和切换：线程上下文切换比进程上下文要快的多；不同App的切换、同一个app内的功能切换
- 线程天生共享内存空间
- 进程开销大【体量大】，线程开销小【体量小】
